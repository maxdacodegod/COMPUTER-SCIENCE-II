...
void reverse_list(Node* head)
{
	...
	Node* next = head->next; //Next one

	while (current != NULL)
	{
		previous = &current; //Previous Has the Head

					/* SHIFT */
					/*
							previous = previous->next; //Previous becomes Current
							current = current->next; // Current becomes next
							next = head->next; //Next becomes Next Next
						}
					}
*/
					//Define your function here
...
Node* reverse_list(Node* head)
{
	...
	return head;
	//previous->next->next->next->next->next->next->next->next->next->next = NULL;
